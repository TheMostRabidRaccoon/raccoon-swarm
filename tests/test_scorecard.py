...
def test_scorecard_counts_and_shape():
    sc = closer.build_scorecard(_digest(), phantom_paths=["positions/ghost.md", "questions/vapor.md"])
...
def test_scorecard_phantom_paths_capped():
    many = [f"positions/p{i}.md" for i in range(50)]
    sc = closer.build_scorecard(_digest(), phantom_paths=many)
    assert sc["persistence_gap"] == 50  # count is the full magnitude
    assert len(sc["filestore"]["phantom_paths"]) == 20  # but the list is capped


def test_scorecard_query_truncated_and_trimmed():
    sc = closer.build_scorecard(_digest(query="x" * 500), phantom_paths=[])
    assert len(sc["query"]) == 200


def test_scorecard_has_no_selfgraded_fields():
    # Judgment fields are excluded by design — mechanical only.
    sc = closer.build_scorecard(_digest(), phantom_paths=[])
...
def test_scorecard_serializes_to_json():
    sc = closer.build_scorecard(_digest(), phantom_paths=["a/b.md"])
...
def test_phantom_claims_names_only_unpersisted(storage):
    ...
    assert closer.find_phantom_claims([], synthesis) == ["positions/ghost.md"]


def test_phantom_claims_dedup_across_rounds_and_synthesis(storage):
    # Same phantom path claimed in a round AND the synthesis appears once.
    all_rounds = [{"claude": "I wrote positions/ghost.md"}]
    synthesis = "As saved, positions/ghost.md now exists."
    assert closer.find_phantom_claims(all_rounds, synthesis) == ["positions/ghost.md"]
